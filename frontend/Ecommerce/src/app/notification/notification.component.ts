    ...
    <div *ngIf="notification" class="notification" [ngClass]="notification?.type">
      <div class="notification-content">
        ...
        <span *ngIf="notification?.link; else plainMessage">
          <a [routerLink]="notification?.link" (click)="closeNotification()">{{ notification?.message }}</a>
        </span>
...
export class NotificationComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.subscription = this.notificationService.notifications$.subscribe(notification => {
      ...
      this.timeout = setTimeout(() => this.closeNotification(), 4000); // Auto-hide after 4 seconds
    });
  }
